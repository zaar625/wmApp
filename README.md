# Work & Manage app 
혹시 내 급여는 근로시간 만큼 제대로 들어오고 있을까, 혹여 실수로 잘못 들어오지 않을까, 라는 생각으로 캘린더에 또 옮겨 적진 않으신가요?<br>
사장님은 우리 직원들이 근태를 잘 하고 있을지, 월말에 급여 계산으로 시간을 할애하고 계시진 않으신가요?<br>
직원들에게는 앱하나로 출퇴근 등록과 동시에 급여계산은 바로바로, 사장님에는 한눈에 인건비 지출내역을 싹 - <br>
더이상 전달사항은 카톡 또는 노트에 적지말아요. 사진과 함께 알림으로 디테일하게 매장 전 직원들에게 전달해보세요.
...
```
#### 2. 브랜치
브랜치의 경우 탭별(기능 별)로 개발합니다. 기능별로 개발된 내용은 develop에 저장하며 develop 브랜치는 QA를 위한 브랜치입니다. 
> optimized 브랜치는 최적화 테스트용 브랜치입니다. 해당 브랜치는 develop에 저장되지 않습니다.(_RAM Bundles and Inline Requires test)<br>
...
로그인 : <br>
- 이메일 유효성 검사, 가입된 유저가 옳바른 정보를 입력하지 않았을 경우 리턴되는 에러값을 이용하여 UI에 표시하고 포커싱이 되도록 구현되었습니다.<br>

회원가입:<br>
- 사용자가 입력하지 않은 필드값이 있을 경우 UI로 안내합니다. 회원가입시 최종적인 필드 값만 필요하므로 ```onChange()``` 함수를 사용하지 않습니다.
